/**
 * Codevertex Afya - Hospital Management Software Quotation PDF generator.
 * Drives the branded client quotation PDF from the markdown source.
 *
 * markdown  --markdown-it-->  HTML body  -->  branded HTML shell
 * (cream cover + clickable TOC + flowing sections, inline CSS, base64 logo)
 *  --puppeteer/Chrome-->  A4 PDF.
 *
 * Mirrors shared-docs/processa-integration/architecture/build/build-pdf.cjs
 * (same pipeline, same Codevertex logo/palette, quotation content instead of
 * an architecture whitepaper). This is a one-off business-development
 * document generator, distinct from treasury-api's runtime fpdf `docs.Document`
 * engine used for tenant-facing invoices/quotations/receipts inside the platform.
 * Run:  pnpm install && node build-pdf.cjs
 */
'use strict';
...
const MD   = path.join(ARCH, 'quotation-source.md');
const OUT  = path.join(ARCH, 'Codevertex-Afya-Hospital-Management-Quotation.pdf');
...
// ── palette: warm cream paper + plum brand (house style, same as processa-integration) ──
...
<div class="cover">
  ...
  <div class="rule"></div>
  <span class="cover-tag">Confidential &middot; Client Quotation</span>
  <h1>Codevertex Afya<br/>Hospital Management Software<br/><span class="arrow">&mdash;</span> Quotation</h1>
  <p class="sub">A tailored quotation for a comprehensive Hospital Management Information System covering
  consultation, laboratory, pharmacy, and billing/finance from one connected patient record &mdash; built
  for Kenyan health facilities, SHA/SHIF and Taifa Care HMIS ready from day one.</p>
  <div class="badges">
    <span class="badge badge-a">Afya Clinic + Inpatient</span>
    <span class="badge badge-b">SHA / SHIF Ready</span>
    <span class="badge badge-c">M-Pesa &amp; KRA eTIMS</span>
    <span class="badge badge-c">Cloud Hosted &middot; Kenya</span>
  </div>
  <div class="divider"></div>
  <div class="meta">
    <div><label>Prepared By</label><span>Codevertex Africa Limited</span></div>
    <div><label>Prepared For</label><span>Prospective Client</span></div>
...
<title>Codevertex Afya - Hospital Management Software Quotation</title>
...
const HDR = `<div style="box-sizing:border-box;width:100%;padding:6px 56px 4px;display:flex;align-items:center;justify-content:space-between;background:${CREAM};font-family:'Helvetica Neue',Arial,sans-serif">
  ...
  <span style="font-size:7.2pt;color:${MUT};text-align:center;flex:1;padding:0 12px">CONFIDENTIAL &nbsp;&middot;&nbsp; Hospital Management Software Quotation &nbsp;&middot;&nbsp; Codevertex Africa Limited</span>
</div>`;
