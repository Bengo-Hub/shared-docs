...
| **Auth-ui: Logout does not clear user / Dashboard still shows** | **Fixed** | **Root cause:** Logout only cleared Zustand store; React Query cache for `['me']` still held the previous user, so `useAuth().user` showed the old user. **Fix:** auth-ui uses a `useLogout` hook that (1) calls POST `/api/v1/auth/logout` (clears server session and cookie), (2) clears the auth store, (3) removes the `['me']` query, (4) redirects to `/`. Dashboard/TopNav/Sidebar use `useLogout()` instead of store `logout`. |
| **Auth-ui: Post-login redirect to service fails (Chrome) / return_to to app not allowed** | **Fixed** | **Root cause:** (1) When `return_to` was the full SSO authorize URL, we used full page redirect; when it was an app URL (e.g. ordersapp), `isValidReturnUrl` rejected it so we fell back to `/`. (2) Cookie clear on logout did not set `Domain`, so the cookie set with `Domain=.codevertexitsolutions.com` was not cleared. **Fix:** auth-ui `isValidReturnUrl` now allows known app origins (ordersapp, books, pos, etc.) and a fallback for sso authorize URL; LoginForm prefers full page redirect for any valid absolute `return_to`; auth-api clears session cookie with `Domain=.codevertexitsolutions.com`. |
| **Auth-ui: Redirect after login to SSO authorize (service-originated)** | **Fixed** | When OIDC params (clientId, redirectUri) are present, LoginForm now builds the authorize URL against the SSO base URL and path `/api/v1/authorize` (not auth-ui origin), so the browser hits auth-api and sends the session cookie. |
| **Treasury-ui: "Session expired" on callback** | **Fixed** | **Root cause:** React Strict Mode double-mount ran the callback effect twice; the second run called `consumeVerifier()` and got null. **Fix:** Callback page uses a module-level guard so `handleSSOCallback` runs only once; store uses `getVerifier()` and only calls `removeVerifier()` after successful token exchange. |
