...
identity and message history, and there's no equivalent to a "shared shortcode" that would make
sense for it. If either requirement isn't met, WhatsApp sends for your account simply don't go out
(every other channel keeps working normally) rather than silently going out under someone else's
number.
