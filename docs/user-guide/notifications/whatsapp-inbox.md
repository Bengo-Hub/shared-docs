...
> **Direct link:** the **WhatsApp Inbox** entry in the sidebar (only visible if you have access —
...
reply to messages sent to it is through software built on the same API, which is exactly what this
inbox is. Nothing changes for your *customers* — they still message an ordinary-looking WhatsApp
number from their own phone as normal.
...
[Why WhatsApp uses fixed templates](message-templates.md#why-whatsapp-uses-fixed-templates) — and
this inbox's composer doesn't send those, so it disables the reply box entirely once the window
closes, with a note explaining why.

The window resets every time the customer messages you again — there's nothing to do on your end
to "reopen" it.
...
real notification — tap it and it opens straight to that conversation, the same way a real
WhatsApp notification would. Enable it from the banner shown inside the inbox itself the first
time you visit, or later from your browser's own site-notification settings if you dismissed it.
