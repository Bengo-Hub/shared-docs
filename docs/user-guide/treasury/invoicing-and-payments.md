...
3. **Recorded manually** — for a payment that happened outside the platform entirely: a bank
   ...
   product's equivalent action) and optionally note a reference — the M-Pesa code sighted, a bank
...
Every recorded payment — whichever path it came through — shows up the same way in
[Reports](reports.md) and in the customer's own payment history.
...
**pay-now link and invoice PDF** as the original invoice notification — a customer is never asked
to pay an overdue invoice without a direct way to do it.
...
from the invoice notification or reminder, not a shortened/retyped version — the link includes a
