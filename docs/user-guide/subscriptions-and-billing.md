...
Manage all of this from **My Organization → Billing** — one tab per product, each showing its
active plan and renewal date, or letting you pick one if there isn't one yet:

![Billing tab — one tab per product, with monthly and service-charge plans](organisation/assets/managing-your-organisation/08-billing.png)

Some products (Ordering, for example) also offer a **service-charge** option alongside monthly
plans — a percentage-of-transaction fee with no fixed monthly cost, useful for a business that
would rather pay proportionally to what it actually sells through the platform than a flat rate.
**Manage** on any tab opens the full plans/billing experience for that product; see
[Managing Your Organisation → Billing](organisation/managing-your-organisation.md) for the tab
itself.
...
A small number of features aren't part of any plan tier at all — they're granted to specific
accounts directly by Codevertex rather than unlocked by upgrading. Inventory's per-branch/outlet
pricing and Aging Stock clearance pricing are examples (see
[Inventory Administration → Add-ons](inventory/administration.md#add-ons)). If a screen shows an
**Add-on** badge instead of a normal "upgrade your plan" prompt, that's the difference — talk to
your account manager rather than looking for a higher tier to buy.
