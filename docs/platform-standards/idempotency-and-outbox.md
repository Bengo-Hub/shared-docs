...
If you write a bare business payload into that column (e.g. while seeding test data or writing a one-off recovery script), `FromJSON` parses it into an empty `Event`, `Subject()` resolves to `"."`, the publish to `"."` "succeeds" against no subscribed stream, and prune-on-publish **deletes the row as if it had been delivered — with zero trace, not even a FAILED row.** This is not a hypothetical: it caused *unrecoverable* production event-history loss for two services (every `erp.*`/`library.*` event since their Go cutover — prod streams showed 0 messages ever, outbox tables were empty, and the fix was forward-only).
