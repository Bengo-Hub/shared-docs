...
- **`source_service` is REQUIRED** for revenue attribution. Each service must use its canonical identifier:
...
- Treasury-api uses `source_service` to attribute revenue per service and generate per-service analytics.
