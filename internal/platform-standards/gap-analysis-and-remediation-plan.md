...
2. **IN PROGRESS 2026-08-06 — controller installed, first-service migration pending.** Sealed Secrets controller (bitnami chart v2.19.1, not External Secrets Operator — no external KMS/Vault exists in this stack, so ESO would add an operator layer for no benefit over Sealed Secrets' simpler in-cluster-keypair model) is live in `kube-system`, verified via `kubeseal --fetch-cert`. `apps/sealed-secrets-manifests/app.yaml` scaffolds directory-sync of future per-service `SealedSecret` files so migrating a new service is just "add a file." Pilot service chosen (`library-api-secrets`) but not yet migrated — the live secret export step is reserved for the user to run directly (touches live secret values), still pending as of this note. No dashboard/UI for Sealed Secrets — that's by design (the decrypting key never leaves the cluster); a future thin admin-panel wrapper was discussed and deferred, not built.
